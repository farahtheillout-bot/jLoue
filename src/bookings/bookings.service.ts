    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (listing.status !== 'PUBLISHED')
          throw new BadRequestException('Listing not bookable');
        ...
        if (listing.bookingUnit === 'DAY' || listing.bookingUnit === 'WEEK') {
          ...
          if (!ok) {
            throw new BadRequestException(
              `Bookings must align to ${h}:00 in ${tz}`,
            );
          }
          ...
          if (listing.bookingUnit === 'WEEK') {
            ...
            if (startLocal.weekday !== 1 || endLocal.weekday !== 1) {
              throw new BadRequestException(
                `Weekly bookings must start/end on Monday in ${tz}`,
              );
            }
          }
        }
        ...
        if (listing.bookingUnit === 'HOUR') {
          ...
          if (!ok) {
            throw new BadRequestException(
              `Hourly bookings must start/end on the hour in ${tz}`,
            );
          }
        }
        ...
        if (durationUnits < listing.minDuration) {
          throw new BadRequestException(
            `Minimum duration is ${listing.minDuration} ${listing.bookingUnit}(s)`,
          );
        }

        if (listing.maxDuration != null && durationUnits > listing.maxDuration) {
          throw new BadRequestException(
            `Maximum duration is ${listing.maxDuration} ${listing.bookingUnit}(s)`,
          );
        }
